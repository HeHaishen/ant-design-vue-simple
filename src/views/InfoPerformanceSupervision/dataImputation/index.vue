 ...
 * @LastEditTime: 2021-01-08 17:08:18
-->
<template>
  <CContent>
    ...
    <template v-slot:contentDetail>
      <div class="c-statistics-title">数据归集统计表</div>
...
export default {
  ...
  methods: {
    ...
    exportHandel () {
      ...
      const that = this
      this.$exportHandel({url:'/merits-server/credit/CreditCollectCount/export',data: this.queryParam},()=>{
        that.loading = false
      })
    },
    ...
    fetch () {
      ...
      send.applyType = '1'
      this.queryParam.body = send
      this.$xttp.InfoPerformanceSupervisionApi.dataPerformanceList(this.queryParam).then((res) => {
        this.loading = false
        if (res.success) {
