 ...
 * @LastEditTime: 2021-01-08 17:00:20
-->
<template>
  <CContent>
    ...
    <template v-slot:contentDetail>
      <div class="c-statistics-title">数据更新统计表</div>
...
export default {
  ...
  methods: {
    ...
    exportHandel () {
      ...
      const that = this
      this.$exportHandel({url:'/merits-server/credit/CreditUpdateCount/export',data: this.queryParam},()=>{
        that.loading = false
      })
    },
    fetch () {
      ...
      // send.applyType = '2'
      this.queryParam.body = send
      this.$xttp.InfoPerformanceSupervisionApi.dataUpdateList(this.queryParam).then((res) => {
        this.loading = false
        if (res.success) {
